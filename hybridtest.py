...
import csv
from bridge import lahrl_solution_to_alns_state
...
from FSMVRP_Tester import FSMVRPTester_PPO
import argparse
import contextlib
import logging
import math
...
def interactive_menu():
    ...
    print("=" * 50)
    print(" LA-HRL to ALNS Hybrid Evaluation Menu ".center(50))
    print("=" * 50)
    ...
    if dataset == "GoldenBC":
        ...
        print("\n" + "-" * 50)
        replay_choice = input(
            "Run Replay Script after testing? (Y/n): ").strip().lower()
        if replay_choice == 'n':
            run_replay = False
    ...
    return dataset, selected_methods, selected_instances, num_to_test, num_runs, run_replay
...
def setup_logger(log_dir: str, log_level: str, timestamp: str) -> tuple[logging.Logger, Path]:
    ...
    log_path = resolved_dir / f"hybrid_run_{timestamp}.log"
    ...
    def _excepthook(exc_type, exc_value, exc_traceback):
        logger.exception(
            "Uncaught exception during hybrid run",
...
def torch_default_device(device):
    target = "cpu" if str(device) == "cpu" else str(device)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Sequential LA-HRL -> ALNS hybrid runner")
    parser.add_argument("--checkpoint_path", type=str,
                        default="./results/train_20nodes", help="Checkpoint directory.")
    parser.add_argument("--checkpoint_epoch",
                        help="Checkpoint epoch.", default="best")
    parser.add_argument("--pomo_size", type=int, default=1000)
    parser.add_argument("--aug_factor", type=int, default=8)
    parser.add_argument("--cuda", action="store_true",
                        default=True, help="Run LA-HRL inference on CUDA if available.")
    parser.add_argument("--log_level", type=str, default="INFO")

    parser.add_argument("--reuse_pool", dest="reuse_pool", action="store_true", default=True,
                        help="Reuse one ProcessPoolExecutor per benchmark file for hybrid ALNS seeds.")
...
def build_tester(args, data, pt_path):
    use_cuda = bool(args.cuda and torch.cuda.is_available())
    env_params = {
        "min_problem_size": int(data["node_xy"].shape[1]),
        "max_problem_size": int(data["node_xy"].shape[1]),
        "pomo_size": int(args.pomo_size),
        "min_agent_num": int(data["agent_capacity"].shape[1]),
        "max_agent_num": int(data["agent_capacity"].shape[1]),
        "utilization_penalty": {"enable": False},
    }
    tester_params = {
        "use_cuda": use_cuda,
        "cuda_device_num": 0,
        ...
        "pomo_size": int(args.pomo_size),
        "model_load": {"path": args.checkpoint_path, "epoch": args.checkpoint_epoch},
        "test_data_load": {"enable": True, "filename": str(pt_path)},
        "augmentation_enable": True,
        "aug_factor": int(args.aug_factor),
        "solution_detail": {"enable": False},
        "csv_export": {"enable": False},
    }
    tester = FSMVRPTester_PPO(
        env_params=env_params, model_params=MODEL_PARAMS_20NODE, tester_params=tester_params)
    tester.env.use_saved_problems(str(pt_path), tester.device)
    torch.set_default_device("cpu")
    return tester


def capture_lahrl_solution(tester, episode_index):
    captured = {}
    original_collect = tester._collect_solution_rows
    original_csv_enable = tester.csv_export_enable

    def _capture(self, episode_index, reset_state, aug_factor, raw_cost, penalized_cost,
        ...
        best_aug_cost = float(
            flat_raw_cost[best_aug_index, best_aug_pomo].item())

        captured["no_aug_cost"] = no_aug_cost
        captured["aug_cost"] = best_aug_cost
        captured["aug_index"] = best_aug_index
        captured["pomo_index"] = best_aug_pomo
        captured["solution"] = self._extract_solution(
            reset_state,
            fleet_history,
            fleet_mask_history,
            route_history,
    ...
    tester._collect_solution_rows = types.MethodType(_capture, tester)
    tester.csv_export_enable = True
    try:
        with torch_default_device(tester.device):
            no_aug_score, aug_score, penalized_no_aug_score, penalized_aug_score = tester._test_one_batch(batch_size=1,
                                                                                                          episode_index=episode_index)
    finally:
        tester._collect_solution_rows = original_collect
        tester.csv_export_enable = original_csv_enable
        torch.set_default_device("cpu")

    captured["returned_no_aug_score"] = float(no_aug_score)
    captured["returned_aug_score"] = float(aug_score)
    return captured
...
def run_hybrid_alns(init_state, run_idx=0, seed=1234, get_config=False):
    ...
    stop = StagnationStop(
        total_iterations, stagnation_patience, min_iterations)
...
def log_instance_table(rows, logger):
    header = f"{'Inst':>4} | {'LA-HRL':>10} | {'Scratch':>10} | {'Hybrid':>10} | {'Best-Known':>10} | {'Gap(Best)':>10} | {'Gap(Paper)':>10} | {'Time(s)':>8} | {'Total iterations':>6}"
    logger.info(header)
    logger.info("%s", "-" * len(header))
    for row in rows:
        gap_b = row.get("hybrid_gap_best", float("nan"))
        gap_p = row.get("hybrid_gap_paper", float("nan"))
        iters = row.get("best_iters", 0)

        row_string = (
            f"{row['instance']:>4} | "
            f"{_format_metric(row['lahr_cost']):>10} | "
            f"{_format_metric(row['scratch_cost']):>10} | "
            f"{_format_metric(row['hybrid_cost']):>10} | "
...
def log_summary(rows, logger):
    lahr = np.array([row["lahr_cost"] for row in rows], dtype=float)
    scratch = np.array([row["scratch_cost"] for row in rows], dtype=float)
    hybrid = np.array([row["hybrid_cost"] for row in rows], dtype=float)
    gaps_best = np.array([row["hybrid_gap_best"] for row in rows], dtype=float)
    gaps_paper = np.array([row["hybrid_gap_paper"]
    ...
    logger.info("Failed instances : %d", int(np.isnan(hybrid).sum()))
    logger.info("Avg LA-HRL cost  : %s",
                _format_metric(float(np.nanmean(lahr)) if not np.isnan(lahr).all() else float("nan")))
    logger.info("Avg scratch cost : %s",
                _format_metric(float(np.nanmean(scratch)) if not np.isnan(scratch).all() else float("nan")))
    logger.info("Avg hybrid cost  : %s",
                _format_metric(float(np.nanmean(hybrid)) if not np.isnan(hybrid).all() else float("nan")))

    gains = scratch - hybrid
    logger.info("Avg gain vs base : %s",
                _format_metric(float(np.nanmean(gains)) if not np.isnan(gains).all() else float("nan")))
...
def collect_hybrid_seed_results(executor, lahr_state, run_idx, active_seeds, hard_timeout_limit, logger, instance_number):
    """Runs one hybrid ALNS job per seed and returns the best completed seed result."""

    future_to_seed = {
        executor.submit(run_hybrid_alns, lahr_state, run_idx, seed): seed
        for seed in active_seeds
    }

    hybrid_cost_raw = float('inf')
    ...
    try:
        ...
        for future in completed_futures:
            ...
            try:
                ...
                all_hybrid_stats.extend(stats)

                if cost < hybrid_cost_raw:
                    hybrid_cost_raw = cost
                    best_hybrid_state = b_state
                    best_hybrid_iters = stats[0].get(
                        "iterations", 0) if stats else 0
            except Exception as e:
                logger.error(
                    f"Sprint failed on Seed {seed} for Instance {instance_number}: {e}")
    except TimeoutError:
        ...
        for future, seed in future_to_seed.items():
            if not future.done():
                future.cancel()
                logger.error(
                    f"Sprint timed out on Seed {seed} for Instance {instance_number}")

    return hybrid_cost_raw, best_hybrid_state, all_hybrid_stats, best_hybrid_iters


def process_benchmark_file(pt_file: Path, raw_pt: Path, method: str, instance_id: str, args, logger, run_idx: int,
                           dataset: str = "GoldenBC", num_to_test: int = None, run_replay: bool = False):
    logger.info("==================================================")
    logger.info("Processing benchmark file: %s", pt_file.name)

    data = torch.load(pt_file, map_location="cpu", weights_only=False)
    instances = build_data_instances(data)
    tester = build_tester(args, data, pt_file)
    ...
    rows = []
    max_cores = len(HYBRID_SEEDS)
    active_seeds = HYBRID_SEEDS[:max_cores]
    hard_timeout_limit = 10000
    shared_executor = None

    if args.reuse_pool:
        logger.info(
            "Reusing one hybrid ALNS process pool with %d workers", max_cores)
        shared_executor = ProcessPoolExecutor(max_workers=max_cores)

    for i, inst in enumerate(instances):
        start_time = time.time()
        try:
            capture = capture_lahrl_solution(tester, episode_index=i)
            extracted_solution = capture["solution"]
            lahr_state = lahrl_solution_to_alns_state(
                extracted_solution, inst, logger=logger, capacity_tolerance=1e-4)
            assert all(route[2] <= inst["capacity"][route[1]] +
                       1e-5 for route in lahr_state.routes)

            lahr_cost_raw = float(lahr_state.objective())

            if shared_executor is None:

                with ProcessPoolExecutor(max_workers=max_cores) as executor:
                    hybrid_cost_raw, best_hybrid_state, all_hybrid_stats, best_hybrid_iters = collect_hybrid_seed_results(
                        executor, lahr_state, run_idx, active_seeds, hard_timeout_limit, logger, i + 1
                    )
            else:

                hybrid_cost_raw, best_hybrid_state, all_hybrid_stats, best_hybrid_iters = collect_hybrid_seed_results(
                    shared_executor, lahr_state, run_idx, active_seeds, hard_timeout_limit, logger, i + 1
                )

            log_directory = Path(logger.handlers[0].baseFilename).parent
            export_operator_stats_csv(
                instance_id, "Hybrid", run_idx, all_hybrid_stats, log_directory)

            if best_hybrid_state is not None:
                export_best_routes_csv(
                    instance_id, "Hybrid", run_idx, best_hybrid_state, log_directory)
                export_for_replay(instance_id, pt_file.name, run_idx,
                                  best_hybrid_state, log_directory, method)

                import subprocess

                summary_path = log_directory / \
                    f"replay_summary_{method}_{instance_id}_run{run_idx}.csv"
                routes_path = log_directory / \
                    f"replay_routes_{method}_{instance_id}_run{run_idx}.csv"
                script_path = Path(
                    "Benchmarks/GoldenBC/scripts/replay_standard_cost.py")

                if script_path.exists() and run_replay:
                    logger.info(
                        f"Auto-running Replay Script for Instance {instance_id}...")
                    try:
                        result = subprocess.run([
                            sys.executable,
                            str(script_path),
                            "--summary-csv", str(summary_path),
                            "--routes-csv", str(routes_path),
                            "--source-pt-filename", pt_file.name
                        ], check=True, capture_output=True, text=True)

                        logger.info(
                            f"Replay successful. Standardized files generated.")
                    except subprocess.CalledProcessError as e:
                        logger.error(
                            f"Replay script crashed!\nError Output:\n{e.stderr}")
                else:
                    logger.warning(
                        f"Could not find replay script at {script_path} or not running replay script. Skipping.")
            lahr_cost = lahr_cost_raw * L
            scratch_cost = float("nan")
            hybrid_cost = hybrid_cost_raw * L

            hybrid_gap_best = ((hybrid_cost - best_known) /
                               best_known * 100.0) if best_known else float('nan')
            hybrid_gap_paper = ((hybrid_cost - paper_cost) /
                                paper_cost * 100.0) if paper_cost else float('nan')

            elapsed = time.time() - start_time
            logger.info(
                "Instance %03d | LA-HRL: %.4f | Scratch ALNS: %.4f | Hybrid: %.4f | Time: %.2fs",
                i + 1, lahr_cost, scratch_cost, hybrid_cost, elapsed,
            )
            ...
            rows.append({
                ...
                "hybrid_gap_best": hybrid_gap_best,
                "hybrid_gap_paper": hybrid_gap_paper,
                "time": elapsed,
                "best_iters": best_hybrid_iters
            })
        except Exception:
            elapsed = time.time() - start_time
            logger.exception(
            ...
            rows.append({
                ...
                "lahr_cost": float("nan"),
                "scratch_cost": float("nan"),
                "hybrid_cost": float("nan"),
                "best_known": best_known,
                "paper_cost": paper_cost,
                "hybrid_gap_best": float("nan"),
                "hybrid_gap_paper": float("nan"),
                "time": elapsed,
                "best_inters": 0
            })
...
def export_for_replay(instance_id, pt_file_name, run_idx, best_state, log_dir, method):
    """Generates strictly formatted summary and route CSVs for the replay script."""

    episode = int(instance_id)
    mode = "hybrid"

    routes_path = Path(log_dir) / \
        f"replay_routes_{method}_{instance_id}_run{run_idx}.csv"
    summary_path = Path(
        log_dir) / f"replay_summary_{method}_{instance_id}_run{run_idx}.csv"

    with open(routes_path, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["episode", "mode", "benchmark_problem_id", "source_pt_filename",
                         "route_index", "vehicle_type", "served_nodes", "distance", "cost"])

        for ri, (path, vt, load) in enumerate(best_state.routes):

            customers = path[1:-1]

            served_nodes_str = " ".join(map(str, customers))

            writer.writerow([
                episode, mode, instance_id, pt_file_name, ri, vt, served_nodes_str,
                0.0, 0.0
            ])
    ...
    with open(summary_path, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["episode", "mode", "benchmark_problem_id", "source_pt_filename",
                         "total_cost", "penalized_total_cost", "customer_count", "total_demand",
                         "served_demand", "avg_load_ratio"])

        obj_cost = float(best_state.objective())
        writer.writerow([
            episode, mode, instance_id, pt_file_name, obj_cost, obj_cost,
            len(best_state.data['demand']) - 1, sum(best_state.data['demand']),
            sum(best_state.data['demand']), 1.0
        ])
...
def main():
    ...
    dataset, selected_methods, selected_instances, num_to_test, num_runs, run_replay = interactive_menu()
    ...
    config = run_hybrid_alns(DummyConfigState(), get_config=True)

    logger.info("==================================================")
    logger.info("           ALNS HYBRID CONFIGURATION              ")
    logger.info("==================================================")
                ...
                for instance_id in selected_instances:
                    ...
                    try:
                        ...
                        file_rows = process_benchmark_file(pt_file, raw_pt, method, instance_id, args, logger, run_idx,
                                                           dataset, num_to_test, run_replay)
    ...
    if num_runs > 1 and all_runs_data:
        logger.info("\n" + "#" * 160)
        logger.info(
            "                                              MULTI-RUN FLUCTUATION SUMMARY (MEAN ± STD DEV)")
        logger.info("#" * 160)
        ...
        for r_idx, run_rows in enumerate(all_runs_data):
            for row in run_rows:
                ...
                if not math.isnan(row.get("lahr_cost", float('nan'))):
                    summary_map[key]["lahr"].append(row["lahr_cost"])
                if not math.isnan(row.get("scratch_cost", float('nan'))):
                    summary_map[key]["scratch"].append(row["scratch_cost"])
                if not math.isnan(row.get("hybrid_cost", float('nan'))):
                    summary_map[key]["hybrid"].append(row["hybrid_cost"])
                if not math.isnan(row.get("time", float('nan'))):
                    summary_map[key]["time"].append(row["time"])

        header = f"{'Inst':>4} | {'Method File':>35} | {'LA-HRL (Mean ± SD)':>20} | {'Scratch (Mean ± SD)':>20} | {'Hybrid (Mean ± SD)':>20} | {'Best Hybrid':>11} | {'Avg Gain':>9} | {'Avg Gap(B)':>10} | {'Avg Gap(P)':>10}"
        logger.info(header)
        logger.info("-" * len(header))

        for (inst, fname), metrics in summary_map.items():
            lahr_mean = np.mean(
                metrics["lahr"]) if metrics["lahr"] else float('nan')
            lahr_sd = np.std(metrics["lahr"]) if metrics["lahr"] else 0.0

            sc_mean = np.mean(metrics["scratch"]
                              ) if metrics["scratch"] else float('nan')
            sc_sd = np.std(metrics["scratch"]) if metrics["scratch"] else 0.0

            hy_mean = np.mean(
                metrics["hybrid"]) if metrics["hybrid"] else float('nan')
            hy_sd = np.std(metrics["hybrid"]) if metrics["hybrid"] else 0.0
            hy_best = np.min(metrics["hybrid"]
                             ) if metrics["hybrid"] else float('nan')

            time_mean = np.mean(
                metrics["time"]) if metrics["time"] else float('nan')
            time_sd = np.std(metrics["time"]) if metrics["time"] else 0.0

            if not math.isnan(sc_mean) and not math.isnan(hy_mean):
                avg_gain = sc_mean - hy_mean
            else:
                avg_gain = float('nan')
            ...
            if not math.isnan(hy_mean) and bk:
                overall_gap_best = ((hy_mean - bk) / bk) * 100.0
            else:
                overall_gap_best = float('nan')

            if not math.isnan(hy_mean) and pc:
                overall_gap_paper = ((hy_mean - pc) / pc) * 100.0
            else:
                overall_gap_paper = float('nan')

            str_lahr = f"{lahr_mean:>9.2f} ± {lahr_sd:<6.2f}" if not math.isnan(
                lahr_mean) else f"{'nan':>20}"
            str_sc = f"{sc_mean:>9.2f} ± {sc_sd:<6.2f}" if not math.isnan(
                sc_mean) else f"{'nan':>20}"
            str_hy = f"{hy_mean:>9.2f} ± {hy_sd:<6.2f}" if not math.isnan(
                hy_mean) else f"{'nan':>20}"

            str_hy_best = f"{hy_best:>11.2f}" if not math.isnan(
                hy_best) else f"{'nan':>11}"
            str_gain = f"{avg_gain:>9.2f}" if not math.isnan(
                avg_gain) else f"{'nan':>9}"
        ...
        for r_idx, run_rows in enumerate(all_runs_data):
            ...
            valid_hybrids = [r["hybrid_cost"] for r in run_rows if not math.isnan(
                r.get("hybrid_cost", float('nan')))]
            if valid_hybrids:
                run_avg = np.mean(valid_hybrids)
                run_averages.append((r_idx + 1, run_avg))
        ...
        if run_averages:
            ...
            logger.info("")
            logger.info(
                f">>> BEST OVERALL RUN: Run {best_run_idx} achieved the lowest average hybrid cost ({best_run_avg:.2f}) <<<")
